...
const OCRSelectionOverlay = ({ 
  ...
  const handleMouseDown = useCallback((e) => {
    console.log('🖱️ Mouse down detected', { isActive, button: e.button, target: e.target });
    
    if (!isActive) {
      console.warn('⚠️ Mouse down ignored - OCR not active');
      return;
    }
    ...
    console.log('✅ Starting selection at viewport coords:', { x, y });
  ...
  const handleMouseUp = useCallback((e) => {
    console.log('🖱️ Mouse up detected', { isSelecting, hasSelection: !!currentSelection });
    
    if (!isSelecting || !currentSelection) {
      console.warn('⚠️ Mouse up ignored - not selecting or no selection');
      return;
    }
    ...
    console.log('📏 Selection dimensions:', currentSelection);
    
    // Only process selections that are reasonably sized
    if (currentSelection.width > 20 && currentSelection.height > 20) {
      console.log('✅ Selection is valid size, capturing...');
      // Capture the selected area
      captureSelection(currentSelection);
    } else {
      console.warn('⚠️ Selection too small:', currentSelection);
    }
  ...
  return (
    <div
      ...
      style={{
        cursor: 'crosshair',
        backgroundColor: 'rgba(59, 130, 246, 0.1)', // Light blue tint - visible but not invasive
        pointerEvents: 'auto',
        position: 'absolute',
        top: 0,
        left: 0,
    ...
    >
      {/* OCR Mode Indicator - Traditional Style */}
      <div className="fixed top-24 left-6 bg-blue-600 text-white px-4 py-2 rounded-lg shadow-lg z-[10000] border border-blue-700">
        <div className="flex items-center space-x-2">
          <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>
          <div>
            <span className="font-semibold">OCR Mode Active</span>
      ...
      {currentSelection && (
        <div
          className="fixed border-2 border-blue-600 bg-blue-500 bg-opacity-20"
          style={{
        ...
        >
          <div className="absolute -top-8 left-0 bg-blue-600 text-white px-3 py-1 text-xs rounded font-medium shadow-md">
